...
| `_VM_BUTTON_CLICKED` | Button 被点击时触发，用 `VM_GetLastClickedButton()` 获取按钮实例 ID |
...
| `hp` | int | 当前血量 |
| `maxhp` | int | 最大血量 |
| `plant_type` | string | 层级类型: `"normal"` `"shield_inner"` `"lilypad"` `"shield_outer"` `"coffee"` |
| `plant_id` | string | 卡片ID，如 `"coffee_bean"` |
| `current_level` | int | 当前星级 |
| `skill` | int | 技能等级 |
| `shape` | int | 形态 |
...
| `VM_GetPlantAt(列, 行, "层级")` | 获取指定格子指定层的植物实例 ID。层级: `"normal"` `"shield_inner"` `"lilypad"` `"shield_outer"` `"coffee"`，`"all"`=任意。没找到返回 -1 |
| `VM_GetLastClickedButton()` | 最后被点击的 Button 实例 ID，没有返回 -1 |
...
| `VM_ApplyPlantLevel(实例ID)` | 手动应用植物的星级/技能/形态，刷新攻防血量等实际数值。`VM_SetProp` 改完 `current_level`/`skill`/`shape` 后需调用 |
...
| `VM_LoadSpriteFrames("文件名", 帧数)` | 加载多帧贴图。帧数>1 时自动均分切割。服务端本地加载，客户端网络懒加载 |
| `VM_CreateButton(x, y, "精灵名", 缩放, idle帧, hover帧, press帧)` | 创建一个按钮。后三个帧号 -1=默认(0,1,2)。点击时触发 `_VM_BUTTON_CLICKED` 块 |
| `VM_SpawnPlantsRandom(x, y, w, h, shape, level, skill, card1, ..., card9)` | 在矩形区域内随机种植。shape/level/skill 共享（-1=默认），cardN 为卡片名（最多9个），每格随机选一种，只种空格子。客户端不可调用 |

### 区域操作

| 函数 | 说明 |
|---|---|
| `VM_SwapPlants(列1, 行1, 列2, 行2)` | 交换两个格子上所有植物的位置 |
| `VM_SwapPlantRects(x1, y1, w, h, x2, y2)` | 交换两个等大矩形区域内所有植物的位置（支持重叠，服务端广播） |
| `VM_CompactColumn(列)` | 列压缩：从上到下，有空缺把下面的往上搬。`-1`=所有列 |
| `VM_CompactColumnRev(列)` | 列压缩反向：从下到上，有空缺把上面的往下搬 |
| `VM_CompactRow(行)` | 行压缩：从左到右，有空缺把右边的往左搬。`-1`=所有行 |
| `VM_CompactRowRev(行)` | 行压缩反向：从右到左，有空缺把左边的往右搬 |

> Swap/Compact 系列函数均为服务端执行后通过 MSG_VM_NOTIFY 广播到客户端，客户端直接跳过。
