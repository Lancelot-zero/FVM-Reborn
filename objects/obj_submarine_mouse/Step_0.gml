...
if hp <= 0 && state != ENEMY_STATE.DEAD{
	...
	if (grid_col < 0 || grid_col >= global.grid_cols || grid_row < 0 || grid_row >= global.grid_rows){
		sprite_index = get_load_sprite("spr_submarine_mouse_land")
		death_anim = 12
	}
	else{
		if global.grid_terrains[grid_row][grid_col].type == "water"{
			sprite_index = get_load_sprite("spr_submarine_mouse")
			death_anim = 12
		}
		else{
			sprite_index = get_load_sprite("spr_submarine_mouse_land")
...
if (grid_col < 0 || grid_col >= global.grid_cols || grid_row < 0 || grid_row >= global.grid_rows) {
	...
	if state != ENEMY_STATE.DEAD{
		if global.grid_terrains[grid_row][grid_col].type == "water"{
			if sprite_index == get_load_sprite("spr_submarine_mouse_land"){
				state = ENEMY_STATE.ACTING
				sprite_index = get_load_sprite("spr_submarine_mouse_enter")
				timer = 0
			...
			if sprite_index == get_load_sprite("spr_submarine_mouse"){
				state = ENEMY_STATE.ACTING
				sprite_index = get_load_sprite("spr_submarine_mouse_enter")
				timer = 0
				audio_play_sound(snd_enter_water,0,0)
