...
if hp <= 0 && state != ENEMY_STATE.DEAD{
	...
	sprite_index = get_load_sprite("spr_orange_prince_mouse")
	move_anim = 10
	attack_anim = 6
	move_speed = 0.3
}
event_inherited();
if grid_col <0 {
	sprite_index = get_load_sprite("spr_orange_prince_mouse_land")
}
if global.is_paused or is_frozen or is_stun{
	exit
}
if state == ENEMY_STATE.ATTACK && sprite_index == get_load_sprite("spr_orange_prince_mouse_frog"){
	timer = 0
...
if state == ENEMY_STATE.ACTING{
	...
	if sprite_index == get_load_sprite("spr_orange_prince_mouse_enter"){
		...
		if timer >= flash_speed * 10 or hp <= 0{
			...
			state = ENEMY_STATE.NORMAL
			sprite_index = get_load_sprite("spr_orange_prince_mouse_frog")
			if hp <= 0{
				sprite_index = get_load_sprite("spr_orange_prince_mouse")
				move_anim = 10
				attack_anim = 6
