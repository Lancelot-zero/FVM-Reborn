...
if (hp <= 0) {
	sprite_index = get_load_sprite("spr_dentist_mouse")
...
if hp > 0 && state != ENEMY_STATE.DEAD{
	if state == ENEMY_STATE.DIG{
		...
		if x <= global.grid_offset_x - 20{
			...
			sprite_index = get_load_sprite("spr_dentist_mouse_appear")
			anim_timer = 0
		}
	}
	if state == ENEMY_STATE.APPEAR{
		...
		if anim_timer >= flash_speed * 14{
			...
			sprite_index = get_load_sprite("spr_dentist_mouse_stun")
			anim_timer = 0
		}
	}
