    ...
    if (mouse_check_button_pressed(mb_left)) {
        ...
        if (can_plant && global.flame >= current_cost) {
			...
			if(global.network.mode=="client"){
				...
				if array_contains([obj_wine_bottle_bomb,obj_aquarius_elve,obj_whisky_bomb,obj_kettle_bomb,obj_coke_bomb,obj_bull_firework,obj_magic_chicken,obj_coffee_grounds,obj_firework_dragon,obj_chili_powder,obj_pineapple_explosive_bread],new_plant.object_index){
			...
			if global.grid_terrains[logical_row][logical_col].type == "normal"{
				if(global.network.mode != "client") audio_play_sound(snd_place1,0,0)
			}
			else if global.grid_terrains[logical_row][logical_col].type == "water"{
				if(global.network.mode != "client") audio_play_sound(snd_enter_water,0,0)
			}
            // 取消选择
            is_selected = false;
            if (selected_preview != noone && instance_exists(selected_preview)) {
