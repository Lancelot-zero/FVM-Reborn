...
if shield_hp > 0 && state != ENEMY_STATE.ACTING{
	sprite_index = get_load_sprite("spr_charge_spring_mouse_shield")
	move_speed = 0.6
}
else if shield_hp <= 0 && state != ENEMY_STATE.ACTING{
	sprite_index = get_load_sprite("spr_charge_spring_mouse")
	move_speed = 0.3
}
if shield_hp <= 0 && not placed && not armor_dropped{
	...
	inst.cgravity = 0.8
	inst.sprite_index = get_load_sprite("spr_mouse_spring_2")
	armor_dropped = true
}
event_inherited();
...
if state == ENEMY_STATE.ATTACK && (array_get_index(can_place_list,target_plant.plant_id) != -1) && !placed && shield_hp > 0{
	sprite_index = get_load_sprite("spr_charge_spring_mouse_place")
	timer = 0
	...
	if timer >= flash_speed * 6 or hp <= 0{
		if hp > 0 && instance_exists(target_plant){
			...
			if shield_hp <= hurt_rate * shield_max_hp{
				spring.sprite_index = get_load_sprite("spr_mouse_spring_2")
			}
			spring.row = grid_row
