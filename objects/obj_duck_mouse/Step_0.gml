...
if (grid_col < 0 || grid_col >= global.grid_cols || grid_row < 0 || grid_row >= global.grid_rows) {
	if hp > maxhp - helmet_hp{
		sprite_index = get_load_sprite("spr_duck_mouse_land_helmet")
	}
	else{
		sprite_index = get_load_sprite("spr_duck_mouse_land")
	}
	death_anim = 12
}
else{
	if global.grid_terrains[grid_row][grid_col].type == "water"{
		if sprite_index == get_load_sprite("spr_duck_mouse_land") or sprite_index == get_load_sprite("spr_duck_mouse_land_helmet"){
		...
		if sprite_index == get_load_sprite("spr_duck_mouse") or sprite_index == get_load_sprite("spr_duck_mouse_helmet"){
			state = ENEMY_STATE.ACTING
			sprite_index = get_load_sprite("spr_duck_mouse_enter")
			timer = 0
			audio_play_sound(snd_enter_water,0,0)
	...
	if hp > maxhp - helmet_hp && state != ENEMY_STATE.ACTING{
		if global.grid_terrains[grid_row][grid_col].type == "water"{
			sprite_index = get_load_sprite("spr_duck_mouse_helmet")
		}
		else{
			sprite_index = get_load_sprite("spr_duck_mouse_land_helmet")
		}
		...
		if global.grid_terrains[grid_row][grid_col].type == "water"{
			sprite_index = get_load_sprite("spr_duck_mouse")
		}
		else{
			sprite_index = get_load_sprite("spr_duck_mouse_land")
		}
...
if hp <= maxhp - helmet_hp && not armor_dropped{
	...
	if sprite_index == get_load_sprite("spr_duck_mouse") || sprite_index == get_load_sprite("spr_duck_mouse_helmet"){
		inst.y += 30
		inst.water = true
	}
	inst.ground_y = y - 45
...
if state = ENEMY_STATE.ACTING{
	if hp > maxhp - helmet_hp{
				sprite_index = get_load_sprite("spr_duck_mouse_enter_helmet")
			}
			else{
				sprite_index = get_load_sprite("spr_duck_mouse_enter")
			}
